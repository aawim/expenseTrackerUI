...
<template>
    <div>
        <form @submit.prevent="applyFilters" class="mb-4 flex flex-wrap gap-3">
            <input type="date" v-model="filters.from_date" class="border px-2 py-1 rounded" />
            <input type="date" v-model="filters.to_date" class="border px-2 py-1 rounded" />
            <input type="number" v-model.number="filters.min_amount" class="border px-2 py-1 rounded w-24" min="0"
                step="0.01" />
            <input type="number" v-model.number="filters.max_amount" class="border px-2 py-1 rounded w-24" min="0"
                step="0.01" />
        ...
        <table class="w-full border-collapse border border-gray-300">
            <thead>
                <tr class="bg-gray-100">
                    <th class="border px-3 py-1 w-25">Date</th>
                    <th class="border px-3 py-1 w-10">Amount</th>
                    <th class="border px-3 py-1 w-20">Category</th>
                    <th class="border px-3 py-1 w-30">Payment Method</th>
                    <th class="border px-3 py-1 w-100">Description</th>
                    <th class="border px-3 py-1 w-50">Actions</th>
                </tr>
            </thead>
            <tbody>
                <tr v-if="loading">
                ...
                <tr v-else v-for="item in expenses" :key="item.id">
                    <td class="border px-3 py-1">{{ item.date }}</td>
                    <td class="border px-3 py-1">{{ Number(item.amount).toFixed(2) }}</td>
                    <td class="border px-3 py-1">{{ item.category?.name || '-' }}</td>
                    <td class="border px-3 py-1">{{ item.payment_method?.name || '-' }}</td>
                    <td class="border px-3 py-1">{{ item.description || '-' }}</td>
                    <td class="border px-3 py-1 space-x-2  text-center">
